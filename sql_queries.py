...
# COPY STAGING TABLES FROM Udacity S3 log data
staging_events_copy = ("""
    copy staging_events
    from {}
    iam_role {}
...
# FINAL TABLES
'''
For songplay_table insert below, join songs and artists tables, left join table with staging_events, and save 
start_time, userId, level, song_id, artist_id, sessionId, location, userAgent.

Convert an epoch value back to a time stamp:
For dates with millisecond epoch, divide by 1000 before adding seconds to date
...
#CREATE SQL QUERY INSERT STATEMENTS TO ADD CONTENT TO TABLES
...
# QUERY LISTS containing queries created above
